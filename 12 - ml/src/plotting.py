...
def plot_sig_bkg_from_np_arrays(features, labels, feature_names, logy=False):
    ...
    for i in range(num_features):
        ...
        if logy:
            plt.yscale('log')
        plt.hist(sig_list[i], density=True, bins=100,
                 histtype='step', label="Signal", color='r')
        plt.hist(bkg_list[i], density=True, bins=100,
                 histtype='step', label="Bkg", color='b')
...
def plot_roc(label, score, save_loc):
    ...
    plt.savefig('../grafi/{}'.format(save_loc))
...
def plot_score(label, score, logy=False):
    ...
    if logy:
        plt.yscale('log')
    plt.hist([score_bkg, score_sig], label=["Bkg", "Sig"],
             density=True, bins=30, histtype='step')
    plt.xlabel('Score')
...
def comp_plot(ptitle, vnames, reald, generd, inputd=None, logy=False):
    ...
    for i in range(feat_dim):
        ...
        plt.yticks([])
        plt.grid(True)
        plt.hist(reald[:, i], density=True, bins=100,
                 histtype='step', color='b', label="Real", range=ax_range)
        plt.hist(generd[:, i], density=True, bins=100, histtype='step',
                 color='r', label="Generated", range=ax_range)
        if inputd is not None:
            plt.hist(inputd[:, i], density=True, bins=100,
                     histtype='step', color='g', label="Latent", range=ax_range)
        plt.xlabel(vnames[i])
        plt.legend()
        if (not (i+1) % 6 and i != 0) or (i+1 == feat_dim):
            plt.savefig(
                'bvae_feat-{}-{}.png'.format(ptitle.lower().replace(" ", "-"), i))
            plt.close()
...
def comp_plot_dict(ptitle, vnames, ddict, logy=False):
    ...
    for i in range(feat_dim):
        ...
        for dlabel, ddata in ddict.items():
            plt.hist(ddata[:, i], density=True, bins=100, histtype='step',
                     color=pcolors[nit], label=dlabel, range=ax_range)
            nit += 1
        plt.xlabel(vnames[i])
        plt.legend()
        if (not (i+1) % 6 and i != 0) or (i+1 == feat_dim):
...
def comp_plot_dict_overlay(ptitle, vnames, ddict, logy=False):
    ...
    for i in range(feat_dim):
        ...
        nit = 0
        for dlabel, ddata in ddict.items():
            plt.hist(ddata[:, i], density=True, bins=100,
                     histtype='step', label=dlabel, range=ax_range)
            nit += 1
    plt.xlabel(vnames[0])
    plt.legend()
    plt.savefig(
        'bvae_feat-{}-{}.png'.format(ptitle.lower().replace(" ", "-"), "overlay"))
    plt.close()
...
def cumhist_plot_dict(ptitle, vnames, ddict, logy=False):
    ...
    for i in range(feat_dim):
        ...
        nit = 0
        for dlabel, ddata in ddict.items():
            plt.hist(ddata[:, i], density=True, cumulative=True, bins=1000,
                     histtype='step', color=pcolors[nit], label=dlabel, range=ax_range)
            nit += 1
        plt.xlabel(vnames[i])
        plt.legend()
        if (not (i+1) % 6 and i != 0) or (i+1 == feat_dim):
...
def cumdiff_plot_dict(ptitle, vnames, ddict):
    ...
    for i in range(feat_dim):
        ...
        nit = 0
        plt.plot(x_val, (Acdf-Bcdf),
                 '-{}'.format(pcolors[nit]), label='{}-{}'.format(Alab, Blab))
        plt.xlabel(vnames[i])
        plt.legend()
        if (not (i+1) % 6 and i != 0) or (i+1 == feat_dim):
            plt.savefig(
...
def cumul_and_diff_plot_dict(ptitle, vnames, ddict):
    ...
    for i in range(feat_dim):
        ...
        nit = 0
        plt.plot(
            x_val, Acdf, '-{}'.format(pcolors[nit]), label='{}'.format(Alab))
        nit = 1
        plt.plot(
            x_val, Bcdf, '-{}'.format(pcolors[nit]), label='{}'.format(Blab))
        nit = 2
        plt.plot(x_val, (Acdf-Bcdf),
                 '-{}'.format(pcolors[nit]), label='{}-{}'.format(Alab, Blab))
        plt.xlabel(vnames[i])
...
def hist_and_rat_plot_dict(ptitle, vnames, ddict):
    ...
    for i in range(feat_dim):
        ...
        nit = 0
        ax.plot(x_val, Acdf,
                '-{}'.format(pcolors[nit]), label='{}'.format(Alab))
        nit = 1
        ax.plot(x_val, Bcdf,
                '-{}'.format(pcolors[nit]), label='{}'.format(Blab))
        nit = 2
        ax2.plot(x_val, (Bcdf/Acdf),
                 '-{}'.format(pcolors[nit]), label='{}-{}'.format(Alab, Blab))
